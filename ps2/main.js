...
//         ...
    for (let i = 0; i < guess.length; i++) {
        ...
        // 2.b. If the letter is in the answer but not in the correct position, add the (additional) class 'present' to the <span> element - I used Chat GPT for this line
        else if (correctAnswer.toUpperCase().includes(letter)) {
        ...
        // 2.e. Append the <span> element to the guess's <div> element - I used Chat GPT for this line
...
inputEl.addEventListener('keydown', (event) => {
    // 1. When the user presses the 'Enter' key, the event listener should:
    if (event.key === 'Enter') {
        // 1.a. Get the value of the input element (which is the guess)  - I used Chat GPT for this line
        ...
        if (guess === correctAnswer) {
            showInfoMessage(`You win! The answer was "${correctAnswer.toUpperCase()}".`);
            inputEl.setAttribute('disabled', true); // Disable the input field
            return;
        }
        // 1.d. If the guess is not the correct answer, then:...
        // 1.d.i. Clear the input element's value
        ...
        // Clear the info message when the user types
        clearInfoMessage();
    }
});
